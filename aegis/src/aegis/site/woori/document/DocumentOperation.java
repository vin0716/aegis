...
public class DocumentOperation {
	...
	public DocumentOperation(Athena athena, TCSession session, Vector<String> vector) {
	...
	public void execute() {
		try {
			...
			TCComponentItemType itemType = (TCComponentItemType)session.getTypeComponent("Document");
			for (int i = 0; i < vector.size(); i++) {
				String item_id = getSerial();
				TCComponentItem item = itemType.create(item_id, "A", "Document", getFileName(vector.elementAt(i)), new String(), null);
