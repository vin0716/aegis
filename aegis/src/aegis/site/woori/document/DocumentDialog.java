package aegis.site.woori.document;
...
import aegis.site.woori.item.ItemOperation;
import athena.origin.AthenaDialogSWT;
import athena.service.ResourceService;

public class DocumentDialog extends AthenaDialogSWT {
	private Vector<String> vector;
	private ListViewer listViewer;
	private List list;
	private Button addButton, removeButton;

	public DocumentDialog(Aegis aegis, Shell shell) {
		super(aegis, shell);
	}
	...
	protected void okPressed() {
		new DocumentOperation(athena, session, vector);
		super.okPressed();
	}

	protected boolean isPrepared() {
		if (vector.size() == 0) {
			MessageDialog dialog = new MessageDialog(shell, getString("SelectFile"), null, getString("SelectFile"), MessageDialog.WARNING, new String[] {"Close"}, 0);
			dialog.open();
			return false;
		}
		return true;
